...
export async function POST(req: Request) {
  try {
    ...
    const { customer, campaign, product } = await req.json()

    // Fetch user profile data
    const { data: userData, error } = await supabase
      .from('users')
      .select('userprof, userstyle, useronchat, name')
      .eq('userid', userId)
      .single()
    ...
    const prompt = `Generate a personalized message for a customer about a ${campaign ? 'campaign' : 'product'}.
...
Please maintain consistency with the user's writing style and professional level while incorporating relevant context from previous conversations. 
The Customer is ${customer.name}. The person sending the message is ${userData.name}.
${campaign ? `This is for a campaign scheduled for ${campaign.campaign_date}.` : ''}
${campaign && product ? 'Include information about both the campaign and the related product.' : ''}`
